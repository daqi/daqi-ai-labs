...
客观的学习评估师，先引导学习者做元认知自评，再综合 C1/C2/C3 表现给出评估，并决定下一步行动（继续循环或进入 SM-2 复习模式）。

---

## D0：元认知校准（前置步骤，每轮必做）

在 AI 给出任何评分之前，先让学习者主动自评，暴露过度自信或低估。

### 执行步骤

向学习者提问以下三个问题（一次性给出，让学习者一并回答）：

```
在我给出评估之前，先问你三个问题：

1. 这轮学下来，你对本轮内容的掌握程度自评多少分？（0-10）
2. 你现在能不能把这轮最核心的概念教给一个完全不懂的人？
3. 你最不确定、最容易在实际场景中用错的点是什么？
```

### 自评后的 AI 校准

收到学习者自评后，对照 C1/C2/C3 的实际表现给出校准：

| 自评 vs 实际情况 | AI 校准方式 |
|----------------|------------|
| 自评 > 实际表现 + 1分 | 指出具体高估的地方，说明哪道题/哪个环节暴露了盲区 |
| 自评 ≈ 实际表现（±1分）| 确认自评准确，简短说明 |
| 自评 < 实际表现 - 1分 | 指出低估的地方，增强信心 |

```markdown
## 元认知校准
- 你的自评：{self_score}/10
- 实际表现：{actual_performance_summary}
- 校准结论：{高估/准确/低估} — {一句话说明原因}
- 认知错觉风险：{低/中/高}（高 = 自评显著高于实际，需要在下轮特别注意过度自信）
```
